...
def make_power_plant_net():
    """
    A Bayes Net representation of a power plant alert system. 
    Name attribute: "alarm","faulty alarm", "gauge","faulty gauge", "temperature". 
    """
...
def set_prob(bayes_net):
    """
    Setting probability distribution for each node in the power plant system.
    Name attribute: "alarm","faulty alarm", "gauge","faulty gauge", "temperature". 
    """
...
def get_game_network():
    """
    Creation of a Bayes Net representation of a game problem with probabilities of skill levels and wins, losses, and draws given in READ_ME.
    Teams - A, B, C | Matches - AvB, BvC, CvA
    """
...
def calculate_posterior_prob(bayes_net):
    """
    Calculation of the posterior distribution of the BvC match given that A won against B and tied C. (this can be changed)
    This returns the probabilities corresponding to win, loss and draw likelihood.
    """
    posterior = [0,0,0]
...
def get_ind1(node,x): #sub-function 1
...
def get_ind2(node,x): #sub-function 2
...
def transform(node): #sub-function 3
...
def get_initstate(bayes_net): #function for initial state with no skill levels given.
...
def Gibbs_sampler(bayes_net, initial_state):
    """
    This is a single iteration of the Gibbs sampling algorithm 
    ...
    This returns the new state sampled from the probability distribution as a tuple of length 6.  
    """
...
def MH_sampler(bayes_net, initial_state):
    """
    This is a single iteration of the MH sampling algorithm given a Bayesian network and an initial state value. 
    initial_state is a list of length 6 where: 
    index 0-2: represent skills of teams A,B,C (values lie in [0,3] inclusive)
    index 3-5: represent results of matches AvB, BvC, CvA (values lie in [0,2] inclusive)    
    This returns the new state sampled from the probability distribution as a tuple of length 6. 
...
def gibbs_iter(bayes_net, initial_state, delta, G):
    ...
    """
    Iterate till convergence and then check match to posterior probability and check efficiency
    """
...
def compare_sampling(bayes_net, initial_state):
    ...
    """
    Comparing Gibbs and Metropolis-Hastings sampling on convergence efficiency. 
    Incorporate sample discard period for faster convergence.
    """
